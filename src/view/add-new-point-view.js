...
import { formatDate, getRandomPositiveInteger } from '../utils.js';

function createDestinationPhotos(destination) {
  const picturesList = destination.pictures.length === 0 ? '' :
    destination.pictures.map((picture) =>
      `<img class="event__photo" src="${picture.src}" alt="${picture.description}">`).join('');
  return `<div class="event__photos-tape">${picturesList}</div>`;
}

function createEventOffersList(event) {
  const isChecked = () => getRandomPositiveInteger(0, 1) ? 'checked' : '';
  const offersList = event.offers.length === 0 ? '' :
...
function createAddNewEventTemplate(event, destination) {
  ...
  const pictures = createDestinationPhotos(destination);
  const offersList = createEventOffersList(event);
...
export default class AddNewPointView {
  constructor({ event, eventDestination }) {
    this.event = event;
    this.destination = eventDestination;
  }

  getTemplate() {
    return createAddNewEventTemplate(this.event, this.destination);
  }
